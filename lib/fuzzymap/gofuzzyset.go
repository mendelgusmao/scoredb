...
func NewFuzzySet(data []string, useLevenshtein bool, gramSizeLower int, gramSizeUpper int, minScore float64) *FuzzySet {
	f := FuzzySet{
...
*/
func (f FuzzySet) Get(value string) []FuzzyMatch {
	results := make([]FuzzyMatch, 0)

	// Check for exact match
	if exactMatch, found := f.exactSet[normalizeStr(value)]; found {
		return []FuzzyMatch{{Word: exactMatch, Score: 1.0}}
	}
...
func (f FuzzySet) findMatchesForGramSize(value string, gramSize int) []FuzzyMatch {
	...
	for i := range matches {
		...
		results = append(results, FuzzyMatch{Word: item.normalizedValue, Score: normScore})
	}
	...
	if f.useLevenshtein {
		newResults := make([]FuzzyMatch, 0)

		for i := range results {
			newResults = append(newResults, FuzzyMatch{Score: distance(results[i].Word, normalizedValue), Word: results[i].Word})
		}
